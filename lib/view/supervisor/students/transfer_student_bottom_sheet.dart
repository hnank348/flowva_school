...
import '../../../cubit/supervisor/student_section/transfer_student_state.dart';
...
class TransferStudentBottomSheet extends StatelessWidget {
  final int studentId;
  ...
  static void show(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => MultiBlocProvider(
        ...
        child: TransferStudentBottomSheet(
          studentId: studentId,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: BlocConsumer<TransferStudentCubit, TransferStudentState>(
        listener: (context, state) {
          if (state is TransferStudentSuccess) {
          ...
          if (state is TransferStudentError) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  state.message,
        ...
        builder: (context, state) {
          return Column(
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          context.tr('transfer_student_title'),
                          style: TextStyle(
                            fontFamily: 'Cairo',
                            fontSize: 16,
              ...
              if (state is TransferStudentLoadingSections)
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: 36),
                  child: Center(child: CircularProgressIndicator()),
                )
              else if (state is TransferStudentSectionsLoaded) ...[
                Text(
                ...
                Button(
                  text: context.tr('confirm_transfer_btn'),
                  icon: Icons.check_circle_outline_rounded,
                  color: const Color(0xFFF59E0B),
                  colorText: Colors.white,
                  height: 50,
                  onPressed: state.selectedSection != null
                      ? () => context.read<TransferStudentCubit>().transferStudent(
                    studentId: studentId,
                    tr: context.tr,
                  )
                      : () {},
                ),
              ] else if (state is TransferStudentSubmitting)
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: 36),
                  child: Center(child: CircularProgressIndicator()),
                ),
  ...
  Widget _buildSectionSelectCard(
    ...
    final bgColor = isSelected
        ? const Color(0xFFF59E0B).withOpacity(isDark ? 0.18 : 0.08)
        : (isDark ? cs.surfaceContainerHigh : cs.surfaceContainerLow);

    return InkWell(
      borderRadius: BorderRadius.circular(18),
      onTap: () => context.read<TransferStudentCubit>().selectSection(sec),
      child: Container(
        ...
        child: Row(
          children: [
            Icon(
              ...
              color: isSelected ? const Color(0xFFF59E0B) : cs.onSurfaceVariant,
              size: 20,
            ),
            const SizedBox(width: 12),
            Expanded(
