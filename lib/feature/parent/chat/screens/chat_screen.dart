...
class ChatScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final conversation = ModalRoute.of(context)!.settings.arguments as ChatConversationModel;

    return BlocProvider(
      create: (context) => ChatCubit()..loadMessages(),
      child: Scaffold(
        backgroundColor: isDark ? AppColors.darkBackground : AppColors.backgroundColor,
        appBar: ChatHeaderWidget(conversation: conversation, onBack: () => Navigator.pop(context)),
        body: SafeArea(
          child: Center(
                      ...
                      builder: (context, state) {
                        return ListView(
                          ...
                          children: [
                            ...
                            ...state.messages.map((m) => ChatMessageBubbleWidget(message: m)),
                          ],
                        );
                      },
                    ),
                  ...
                  BlocBuilder<ChatCubit, ChatState>(
                    builder: (context, state) {
                      return MessageComposerWidget(
                        ...
                        onChanged: (val) => context.read<ChatCubit>().updateDraftStatus(val),
                        onSend: (val) => context.read<ChatCubit>().sendMessage(val),
                      );
                    },
                  ),
                ],
